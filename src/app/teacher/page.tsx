...
export default function TeacherPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    const doneSummaries = uploadedFiles.filter((f) => f.status === "done");
    if (!title || (!doneSummaries.length && !additionalContext.trim())) return;

    setLoading(true);

    const parts: string[] = doneSummaries.map((f) => f.summary);
                    ...
                    <TableRow key={s.id} className="border-border/20 hover:bg-muted/30 group">
                      ...
                      <TableCell className="py-3">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            className="group/del h-7 w-7 flex items-center justify-center rounded-md hover:bg-destructive/10 transition-colors"
                            title="Delete student"
                            onClick={async () => {
                              if (!confirm(`Remove ${s.firstName} ${s.lastName}?`)) return;
                              const res = await api(`/api/students/${s.id}`, { method: "DELETE" });
                              if (res.ok) setStudents((prev) => prev.filter((st) => st.id !== s.id));
                            }}
                          >
                            <svg className="h-3.5 w-3.5 text-muted-foreground group-hover/del:text-destructive transition-colors" style={{ transitionProperty: "color" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
                          </button>
                        </div>
